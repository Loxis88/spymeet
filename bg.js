console.log("Background script loaded. VERSION: DEBUG-008");
...
async function handleSummarization(transcript, sendResponse) {
  try {
    await log("Background: Starting summarization (v8)...");
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (isQuotaError && i < retries) {
        ...
        await log(
          `Quota hit (429). Retrying in ${delay / 1000}s... (Attempt ${i + 1}/${retries})`,
        );
...
async function callGemini(text, apiKey) {
  ...
  const prompt = `
    Ты мастер слова который пишет коротко и по делу аналируя запись субтитров созвона
    ты пишешь краткое изложение встречи для наемного сотрудника которого не было на встречи отмечая все ключевые детали
    что бы сотрудник быстро мог погрузиться в суть
